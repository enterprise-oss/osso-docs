...
To best serve our SAML SSO users, we'd only send users who belong to an onboarded SAML customer to Osso, and we wouldn't require a user to remember that they use SAML SSO to sign in. Frankly this issue has become _incredibly fraught_. Non-SAML users hate when you split a login form into two steps, and they usually have no idea why anyone would implement such a poor UX. But there's no perfect solution if we want to serve all of our users well, just the least bad solution, and we'll do our best to make our login flow frustrate the least number of users - engineering is, after all, about tradeoffs.  
