...
Now we can log in again using an `example.com` email address, and we'll skip over Osso's hosted login page. The downside of this approach is that Osso will display an error if there is not a SAML configured customer for the supplied domain. In an ideal world, we'd only send users who belong to an onboarded SAML customer to Osso. There's lots of ways to approach this - perhaps we can take the same approach to the multi-tenant login form from the previous section, and have our first login form submit return to the client a string representing the tenant's valid auth mechanisms.
