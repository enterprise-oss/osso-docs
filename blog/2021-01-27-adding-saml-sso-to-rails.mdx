...
**SP Entity ID**  - Sometimes called the Audience URI, this is a unique identifier for the tenant in your application. This can get hairy! Google requires that they be unique for a customer, so you can't just use domain. Azure won't let you use a UUID. And Ping won't let you use a url. Those are just some of the edge cases we've found!
...
Now we can log in again using an `example.com` email address, and we'll skip over Osso's hosted login page. The downside of this approach is that Osso will display an error if there is not a SAML configured customer for the supplied domain. In an ideal world, we'd only send users who belong to an onboarded SAML customer to Osso, and we wouldn't require a user to remember that they use SAML SSO to sign in. We could take the same approach to the multi-tenant login form from the previous section, and have our first login form submit an email address, and return to the client a string representing the user's valid auth mechanisms, such that we could redirect to Osso. Since we already sort of covered that, let's take a look at Osso's React library, which includes a `<SamlLogin />` component.
