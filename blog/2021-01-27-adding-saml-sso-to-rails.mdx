...
```bash
$ yarn install @enterprise-oss/osso
```

And we need to wrap any of our Osso components in an `<OssoProvider />`, passing a `baseUrl` to the client options:

```javascript title="javascripts/packs/index.jsx" {2-6,13}
const App = () => (
  <OssoProvider
    client={{
